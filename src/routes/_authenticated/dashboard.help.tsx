...
import { HelpCircle, ChevronDown, Camera, WifiOff, MapPin, FileImage } from "lucide-react";
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 font-semibold uppercase mt-0.5">
          Frequently asked questions and user guides for NRLM cadres
        </p>
      </div>
      ...
      <div className="space-y-3.5">
        {FAQS.map((faq) => {
          ...
          return (
            <div
              key={faq.id}
              className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden transition-all hover:shadow-md"
            >
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-sm font-extrabold text-slate-800">{faq.qHi}</h3>
                    <h4 className="text-xs font-semibold text-slate-400 mt-0.5">{faq.qEn}</h4>
                  </div>
                </div>
                <ChevronDown
                  className={cn(
              ...
              <div
                className={cn(
                  "border-t border-slate-50 bg-slate-50/20 px-5 transition-all duration-300 ease-in-out overflow-hidden max-h-0 opacity-0",
                  isExpanded && "max-h-[300px] py-4.5 opacity-100",
                )}
              >
                <div className="space-y-3 text-xs leading-relaxed text-slate-600 font-medium">
                  <div>
                    <p className="font-bold text-slate-800 mb-0.5">उत्तर:</p>
                    <p>{faq.aHi}</p>
                  </div>
                  <div className="border-t border-slate-100/50 pt-2.5">
                    <p className="font-bold text-slate-500 mb-0.5">Answer:</p>
                    <p>{faq.aEn}</p>
                  </div>
                </div>
              </div>
      ...
      {/* Accessability Tool Tips / Govt Standards Badge */}
      <div className="rounded-2xl border border-emerald-100 bg-emerald-50/25 p-5 flex flex-col md:flex-row gap-4.5 items-center justify-between">
        <div className="space-y-1 text-xs text-emerald-800 font-medium leading-normal">
          <h4 className="font-black text-emerald-950 uppercase tracking-wide">
            सरकारी दिशानिर्देशों का अनुपालन / Government Guidelines Compliance
          </h4>
          <p>
            This portal follows Indian Government Web Standards (GIGW / UX4G). Accessibility
            features (high-contrast text ratio, alt tags, keyboard tab index navigation, bilingual
            labels) are built-in for rural accessibility.
          </p>
        </div>
        <div className="rounded-full bg-emerald-500 text-white font-black text-xs px-3.5 py-1.5 shrink-0 shadow-sm border border-emerald-400/20">
          WCAG 2.1 Compliant
        </div>
      </div>
    </div>
