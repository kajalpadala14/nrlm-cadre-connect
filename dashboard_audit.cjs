// Dashboard Data Audit Script — CommonJS
...
async function main() {
  ...
  const { data: cadreRoles, count: cadreCount } = await supabase
    .from("user_roles").select("user_id", { count: "exact" }).eq("role", "cadre");
  console.log("\n=== CADRES ===");
  ...
  if (cadreIds.length > 0) {
    const { data } = await supabase.from("profiles")
      .select("id, full_name, block_id, cadre_type").in("id", cadreIds);
    profiles = data || [];
  }
  ...
  // All attendance
  const { data: attAll } = await supabase.from("attendance").select("cadre_id, date, status, block_id")
  ...
  attUnmapped.forEach(a => console.log(`  ORPHAN ATT: block_id=${a.block_id}`));
  ...
  console.log("Dashboard query uses:", today);
  if (!allDates.includes(today)) {
    console.log("!!! NO ACTIVITIES EXIST FOR TODAY. 'Activities Today' and 'Villages Covered' will show 0.");
    console.log("Activities exist on:", allDates.join(", "));
  }

  // Attendance dates
  const attDates = [...new Set(attAll?.map(a => a.date) || [])].sort();
  console.log("\nAll attendance dates:", attDates);
  if (!attDates.includes(today)) {
    console.log("!!! NO ATTENDANCE RECORDS FOR TODAY. Attendance KPI will show 0.");
  }
